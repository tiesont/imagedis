...
namespace ImageDis
{
    class ImageDisMiddleware : OwinMiddleware
    {
        ...
        private async Task GetImage(IOwinContext context)
        {
            ...
            // parse the params
            var param = new ImageDisParameters(context.Request.Query);
            var parts = _options.TransformImageProvider.GetParameterAsParts(param);
            ...
            var newFile = await _options.TransformImageProvider.TransformImage(original.Stream, param);
